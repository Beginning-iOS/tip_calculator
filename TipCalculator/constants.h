//
//  constants.h
//  TipCalculator
//
//  Created by Mark Haskamp on 11/2/13.
//  Copyright (c) 2013 Mark Haskamp. All rights reserved.
//

#ifndef TipCalculator_constants_h
#define TipCalculator_constants_h

NSString *TOTAL_BILL_SECTION_HEADING = @"Tip Calculator";
NSString *BILL_SPLITS_SECTION_HEADING = @"Bill Splits";
NSString *TIP_CALCULATOR_SECTION_HEADING = @"Tip Percentage";

NSString *TOTAL_BILL_CELL_ID     = @"TotalBillCell";
NSString *BILL_AMOUNT_CELL_ID    = @"BillAmountCell";
NSString *TIP_PERCENTAGE_CELL_ID = @"TipPercentageCell";


int BILL_AMOUNT_TAG_NUMBER  = 104;
int TIP_AMOUNT_TAG_NUMBER   = 102;
int TOTAL_AMOUNT_TAG_NUMBER = 103;
int DELETE_ROW_TAG_NUMBER   = 105;

int TIP_PERCENTAGE_LABEL_TAG_NUMBER  = 201;
int TIP_PERCENTAGE_SLIDER_TAG_NUMBER = 202;
int TOTAL_BILL_TOTAL_TAG_NUMBER      = 203;

int ADD_BUTTON_TAG_NUMBER = 301;


#endif
